...
def join_people(emcee, people):
    return emcee + '、' + people if emcee else people
...
def extract_date(str):
    try:
        ...
        matches = re.findall("(\d{0,2})月(\d{0,2})日", str)
        if len(matches[0][1]) == 2:
            return time.strftime("%Y-", time.localtime()) + '0' + matches[0][0] + '-' + matches[0][1] + ' '
        else:
            return time.strftime("%Y-", time.localtime()) + '0' + matches[0][0] + '-0' + matches[0][1] + ' '
...
clear()
sheet = load_workbook("C:/Users/xiaomi/Desktop/source.xlsx").active
